...
///<summary>
/// Elabora le richieste passate
/// </summary>
/// <param name="parsed">Opzioni passate da riga di comando</param>
let run (parsed: options) =
    let cc = CookieContainer()

    let cookie = Cookie("session", parsed.cookie, "/", "adventofcode.com")

    cc.Add cookie

    let year =
        match parsed.year with
        | 0 -> DateTime.Now.Year
        | _ -> parsed.year

    let url = sprintf "https://adventofcode.com/%d/day/%d/input" year parsed.day
    let response = Http.RequestString(url, cookieContainer = cc)
